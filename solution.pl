...
stryj(X,Y) :- ojciec(Z, Y), brat(X, Z), mê¿czyzna(X), Z\=X.
...
teœciowa(X, Y) :- mê¿czyzna(Y), kobieta(X), matka(X, A), ma³¿eñstwo(A, Y).
szwagier(X, Y) :- mê¿czyzna(X), ma³¿eñstwo(Y, A), brat(X, A), kobieta(A).
szwagier(X, Y) :- mê¿czyzna(X), ma³¿eñstwo(A, Y), brat(X, A), kobieta(A).
szwagier(X, Y) :- mê¿czyzna(X), siostra(A, Y), ma³¿eñstwo(X, A), kobieta(A).
szwagier(X, Y) :- mê¿czyzna(X), siostra(A, Y), ma³¿eñstwo(A, X), kobieta(A).
